...
impl Keyboard {
    ...
    pub fn new (connection: &xcb::Connection) -> (Keyboard, u8, u8) {
        ...
        let (first_ev, first_er) = match connection.get_extension_data(xcb::xkb::id()) {
            ...
            None => { panic!("could not get xkb extension data"); }
        };

        {
            ...
            match cookie.get_reply() {
                Ok(r) => {
                    if !r.supported() {
                        panic!("required xcb-xkb-{}-{} is not supported",
                                xkb::x11::MIN_MAJOR_XKB_VERSION,
                                xkb::x11::MIN_MINOR_XKB_VERSION);
                    }
                }
...

        {
            ...
            let cookie = xcb::xkb::select_events_checked(&connection,
        ...
        let keymap = xkb::x11::keymap_new_from_device(&context, &connection,
                device_id, xkb::KEYMAP_COMPILE_NO_FLAGS);
    ...
    pub fn make_key_event(&self, xcb_ev: &xcb::KeyPressEvent, press: bool) -> Event {
        ...
        if press {
            Event::KeyPress (
                self.get_keysym(xsym), code,
                self.state.borrow().key_get_utf8(xcode),
            )
        }
        else {
            Event::KeyRelease (
                self.get_keysym(xsym), code, String::new()
            )
        }
    }
    ...
    fn mod_active(&self, name: &str) -> bool {
        ...
        self.state.borrow().mod_index_is_active(ind, xkb::STATE_MODS_DEPRESSED)
    }
...
fn build_keysym_map() -> HashMap<u32, key::Sym> {
    ...
    map.insert(xkb::KEY_XF86KbdBrightnessDown,     key::Sym::KeyboardBrightnessDown);
